...
const BlogCard = ({ blogData, size }) => {
    ...
    const { blogImage, date, blogTitle, blogDescription } = blogData
    return (
        <div>
            <div className={` w-full border rounded-tl-3xl border-neutral-100 shadow-sm hover:shadow h-full`}>
                ...
                <div className='bg-green-700 p-4 space-y-2'>
                    ...
                    {
                        size && <button className='text-white hover:text-opacity-80 flex items-center gap-2 border rounded border-white px-4 py-2 hover:border-opacity-80 mt-2'>Read Full <FaArrowRight></FaArrowRight></button>
                    }
                </div>
            </div>
        </div>
